...
class FakeVectorStore:
    ...
    def search(self, search_input, n_results: int):
        ids = search_input.get("id") if isinstance(search_input, dict) else search_input["id"]
...
def test_build_queries_then_run_search_with_mock_vectorstore(tmp_path, monkeypatch):
    ...
    combined_file = config.paths.search_results_isco_file.with_name("search_results_combined.csv")
    assert combined_file.exists()

    isco_results = pd.read_csv(config.paths.search_results_isco_file, dtype=str)
    assert set(["id", "pred1", "pred1label", "match_top_1"]).issubset(isco_results.columns)
