...
3. *Build query files*: The Q2 2024 Nigeria Labour Force Survey data is pre-processed to three columns: [`id`,`query`,`pre_validated`]. The `id` column is a joining variable, `query` contains the free text, and `pre_validated` is the enumerator's 4-digit ISIC or ISCO code.
...
## Search results
The search results generated by the pipeline will be saved in the `/outputs` folder with the name of the coding scheme appended to the filename e.g. `search_results_isic.csv`. There are 9 variables in the search results:

- `id`: joining variable
- `query`: free text survey response
- `pre_validated`: 4-digit ISCO/ISIC code assigned by the enumerator 
- `query_id`: joining variable
- `prediction_1`: top-1 prediction
- `score`: cosine similarity score between the query and vector store entry
- `prediction_2`: top-2 prediction
- `prediction_3`: top-3 prediction
- `match_top_1`: Boolean operator where TRUE is `prediction_1` == `pre_validated`. This can be filtered as the threshold for automatic coding.

## Optional: Add manually codes cases to the knowledgebase
The cases where `match_top_1` == FALSE can be manually coded using the 4-digit ISCO/ISIC candidate codes predicted by the tool. When completed these labelled examples can be added to the original knowledgebase. To do this you must update the `config.json` with the path to the manually coded cases and run:
