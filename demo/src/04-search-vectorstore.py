...
search_results = vector_store.search(search_input, n_results=15)
...
search_results_unique = search_results_unique.merge(
    search_results_scores,
    left_on=["query_id", "doc_id"],
    right_on=["query_id", "doc_id"],
)
search_results_unique["rank"] = search_results_unique.groupby("query_id")[
    "doc_id"
].cumcount()
search_results_0 = search_results_unique[search_results_unique["rank"] == 0].rename(
    columns={"doc_id": "prediction_1"}
)
search_results_1 = search_results_unique[search_results_unique["rank"] == 1].rename(
    columns={"doc_id": "prediction_2"}
)
search_results_2 = search_results_unique[search_results_unique["rank"] == 2].rename(
    columns={"doc_id": "prediction_3"}
)
combined_results = search_results_0.merge(
    search_results_1[["query_id", "prediction_2"]], on="query_id", how="left"
)
combined_results = combined_results.merge(
    search_results_2[["query_id", "prediction_3"]], on="query_id", how="left"
)
combined_results = combined_results[
    ["query_id", "prediction_1", "score", "prediction_2", "prediction_3"]
]
